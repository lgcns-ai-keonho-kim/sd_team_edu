...
class AsyncWorkerBase:
    ...
    async def run_forever(self) -> None:
        """비동기 작업 루프를 실행한다."""
        # TODO: 비동기 루프 생명주기, 예외 처리, 종료 조건을 정의한다.
        raise NotImplementedError("비동기 워커 실행 루프를 구현해야 합니다.")

    async def fetch_job(self) -> dict[str, Any] | None:
        """작업을 비동기로 가져온다."""
        # TODO: 큐/스토리지에서 작업을 비동기로 가져오는 로직을 정의한다.
        raise NotImplementedError("비동기 작업 조회 로직을 구현해야 합니다.")
    ...
    async def handle_job(self, payload: dict[str, Any]) -> None:
        ...
        Args:
            payload: 작업 페이로드.
        """
        # TODO: 비동기 작업 처리 로직과 체크포인트 연계를 정의한다.
        raise NotImplementedError("비동기 작업 처리 로직을 구현해야 합니다.")
